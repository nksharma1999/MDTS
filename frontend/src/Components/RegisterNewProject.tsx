...
//                 const key = moduleName.replace(/\s+/g, "_").toLowerCase();
//                 return (
//                   <Col span={24} key={key}>
...
export const RegisterNewProject: React.FC = () => {
  ...
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [addCompanyPopupOpen, setAddCompanyPopupOpen] = useState<boolean>(false);
  const [orderedModuleNames, setOrderedModuleNames] = useState([]);
  const [formData, setFormData] = useState<{ [key: string]: string }>({});
  const [errors, setErrors] = useState<{ [key: string]: string }>({});
  const steps = [
    { id: 1, title: "Project Parameters" },
  ...
  const requiredFields: { [key: number]: string[] } = {
    ...
    2: ["state", "district", "nearestTown", "nearestAirport", "nearestRailwayStation"],
    3: ["mineOwner", "dateOfH1Bidder", "cbdpaDate", "vestingOrderDate", "pbgAmount"],
    4: Object.values(orderedModuleNames).map((moduleName) => moduleName)
  };
    ...
    switch (currentStep) {
      ...
      case 4:
        return (
          <Form layout="horizontal">
            <Row gutter={[16, 16]}>
              {(Array.isArray(orderedModuleNames) ? orderedModuleNames : []).map((moduleName: any) => {
                    ...
                    <Row className="module-item" gutter={[16, 16]} align="middle">
                      ...
                      <Col span={18}>
                        <Select
                          ...
                          onChange={(value) => handleRowChange(value, key)}
                          style={{ width: "100%" }}
                          placeholder={`Select ${moduleName.name}`}
                        >
                          <Option value="Yes">Yes</Option>
                          <Option value="No">No</Option>
                        </Select>
